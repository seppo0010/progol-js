...
/*			PROGOL initialisation functions			   */
/*			-------------------------------			   */
...
ITEM bfores,bclauses,lib,lib1,psymtlib,fsymtlib,spies,commutes,repeats,labels;
ITEM hmodes,bmodes,determs,hypothesis,hyp1,idot0,store,sstack,indexed,osplit;
ITEM types,op,allpsyms,nums,fores,catoms,clt,cle,sple,fhead,gcall,eof,ptog;
ITEM psamps,bpsamps,costs,prune,constraint,oldops,oldop;
double noiselim,clim;
LONG pprune0,inflate,cthresh;
...
PREDICATE cover,condition,conditioning,memoing,posonly,reductive,sampling,
	searching,splitting,tracing,full_pruning,solving;
PREDICATE pdebug,namecmp,noquotes,reconsult,mseen;
STRING editor;


ITEM tag=(ITEM)0x40276478;
...
c_open()
	...
	spsyms=b_spcreate(13l);		/* Superset of predicate and function symbols */
	...
	oldops=i_create('f',(POINTER)f_zero(f_create(64l))); /* Operators */
	...
	spatoms=b_spcreate(13l);	/* Ground atom bit superset */
	spcls=b_spcreate(13l);		/* Clause superset */
	...
	solving=FALSE;			/* Solving constraint? */
	...
	constraint=B_EMPTY;		/* Constraint predicates */
	...
	sstack=(ITEM)NULL;		/* Used by l_record */
...
struct setrec settings[] = {
	"c", REAL, (POINTER)&clim,
	"compress", NATURAL, (POINTER)&cthresh,
...
c_reset()
	...
	inflate=101l;	/* Example inflation */
	cthresh=0l;	/* Compression threshold */
...
/* n_init - initialise the Prolog numerical functions.
 *	Initialises 'fsymtlib'.
 */
...
n_init()
	...
	struct fnstruc *cptr;
	LONG psym;
	/* Index C numerical functions */
	for(cptr=numfns;cptr->name;cptr++) {
	  psym=QP_ston(cptr->name,cptr->nargs);
	  *y_ins(psym,fsymtlib)=cptr->fno;
	}
}
