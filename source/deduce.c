	...
	if(negq) {
	  ...
	  LIST_DO(ccl1,*entry)
	    if(b_memq(cno1=ccl1->extra,bclauses)) {
	      if(l_length(cl=F_ELEM(0l,ccl1))==2 && cl_groundq(cl)) {
			/* Cover test atoms */
...
cl_vcnt(clause)
	ITEM clause;
	{
	ITEM atom,result=Y_EMPTY,term;
	LIST_DO(atom,clause)
	  TERM_DO(term,atom)
	    if(term->item_type== 'v')
	      (*y_ins((LONG)I_GET(term),result))++;
	  TERM_END
	LIST_END
	return(result);
}

/* d_qreduces - true iff given clause C has a redundant body atom A
 *	where vars(A) subseteq vars(C-A).
 */
...
d_qreduces(clause)
	ITEM clause;
	{
	ITEM cclause0=i_tup4(clause,i_dec(I_INT(cl_vmax(clause))),(ITEM)NULL,
		(ITEM)NULL),head0,retract=Y_EMPTY,atom, pseen=B_EMPTY,
		entry=(ITEM)NULL,term,bvars,clause1,vcntcl,vcntcl1,
		catoms0,cle0,clt0,nums=L_EMPTY;
	register LIST *elemp;
	LONG cno=(LONG)I_GET(F_ELEM(2l,spcls));
	PREDICATE result=FALSE,acond;
	vcntcl=cl_vcnt(clause);
	ct_bvars(head0=l_pop(clause),bvars=B_EMPTY);
	LIST_DO(atom,clause) /* Skolemise variables from head in body */
	  TERM_DO(term,atom)
	    if(term->item_type== 'v'&&b_memq((LONG)I_GET(term),bvars))
	      term->item_type= 'k';
	  TERM_END
	LIST_END
	cl_skolem(clause1=i_copy(clause),TRUE);
	catoms0=catoms; cle0=cle; clt0=clt;
	...
	cl_psfirstarg(pseen);
	if(*f_ins(cno,catoms))  {
	  b_uni(entry=b_copy(*f_ins(cno,catoms)),bclauses);
	  i_swap(bclauses,entry);
	}
	l_push((ITEM)NULL,clause);
	LIST_DO(atom,clause)	/* Test redundancy for each body atom */
	  if(atom) {
	    vcntcl1=i_copy(vcntcl);
	    acond=TRUE;
	    TERM_DO(term,atom)	/* Test vars(A) subseteq vars(C-A) */
	      if(term->item_type== 'v')
	        if(!(--(*y_ins((LONG)I_GET(term),vcntcl1)))) {
		  acond=FALSE;
		  break;
	        }
	    TERM_END
	    ...
	    if(acond) {
	      b_rem(cno,bclauses);
	      if(interp_quest(cclause0)) {result=TRUE; break;}
	      else b_add(cno,bclauses);
	    }
	    cno++;
	  }
...
d_sample(psym,n)
	...
	while(n-- >0l) {
	  if(interp(call,TRUE,TRUE)) {
	    atom1=ct_skolem(p_copy(atom,(BIND)term_stack0,FALSE),TRUE);
