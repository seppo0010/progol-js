...
#define BREAK_PUSH	break1.clause=clause; break1.reinterp=reinterp; \
			...
			break1.env_stack=env_stack; break1.nres=nres; \
...
#define BREAK_POP 	clause=break1.clause; reinterp=break1.reinterp; \
			...
			env_stack=break1.env_stack; nres=break1.nres; \
...
plg_debug(point,env)
	...
	if(skipping && (point==DONE||point==FAIL) && env==env1)
	    skipping=FALSE;
	if(!skipping) {
	  /* Extra code to show parent and called claused
	  if(point==DONE) {
		printf("PARENT(%d): ",env->parent->clause->extra);
		ccl_print(env->parent->clause);
		printf("CALLED(%d): ",env->clause->extra);
		ccl_print(env->clause);
	  }
	   */
